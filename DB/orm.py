...
class Bulletinimage(db.Model):
    ...
    bulletin = db.relationship(
        u'Bulletin',
        backref=db.backref('bulletinimages', cascade="all, delete-orphan"))
...
class Institution(db.Model):
    ...
    def __init__(self, name, agespan_id, area_id, address, location, website,
                 telephone, feedesc, timeopen, timeclose, feetype_id,
                 longitude, latitude, featuredesc):
        self.name = name
...
class InstitutionFeature(db.Model):
    institution_id = db.Column(
        db.ForeignKey(u'institution.id'), primary_key=True)
    feature_id = db.Column(db.ForeignKey(u'feature.id'), primary_key=True)

    institution = db.relationship(
        u'Institution',
        backref=db.backref(
            'institutionfeatures', cascade="all, delete-orphan"))
...
class Institutionimage(db.Model):
    ...
    institution = db.relationship(
        u'Institution',
        backref=db.backref('institutionimages', cascade="all, delete-orphan"))
...
class School(db.Model):
    ...
    def __init__(self, name, area_id, teachdesc, address, schooltype_id,
                 website, distinguish, leisure, threashold, partner, artsource,
                 feedesc, longitude, latitude):
        self.name = name
        self.area_id = area_id
...
class SchoolFeature(db.Model):
    ...
    school = db.relationship(
        u'School',
        backref=db.backref('schoolfeatures', cascade="all, delete-orphan"))
    feature = db.relationship(u'Feature')
...
class Schoolimage(db.Model):
    ...
    school = db.relationship(
        u'School',
        backref=db.backref('schoolimages', cascade="all, delete-orphan"))
